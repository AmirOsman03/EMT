...
public class CountryController {
    ...
    @Operation(summary = "Search countries by continent", description = "Returns a list of countries that belong to the specified continent")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "List of countries by continent",
                    content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = DisplayCountryDto.class))))
    })
    @GetMapping("/searchByContinent")
    public List<DisplayCountryDto> findByContinent(@RequestParam String continent) {
        return countryApplicationService.findByContinent(continent);
    }
}